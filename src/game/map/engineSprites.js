...
function cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, kind, pass = 'all') {
  // Passes de cache : 'back' = statique sous les animations · 'anim' = tout ce qui lit `now`
  // · 'front' = statique par-dessus. En 'all' tous les booléens sont vrais → dessin identique.
  const dBack = pass === 'all' || pass === 'back';
  const dAnim = pass === 'all' || pass === 'anim';
  const { cp, glow } = cosmicBase(ctx, ox, oy, sw, sh, px, band); // sol no-op + palette (aucun dessin réel)
  ...
  if (fam === "dome") {
    ...
    if (dAnim) dot(0.5, 0.2, 0.028 + 0.012 * pulse, cp.lite); // rayon en `pulse` (now) → animé
  } else if (fam === "hall") {
    if (dBack) {
...
function drawEngineSpriteCore(t, x, y, w, h, now, pass = 'all') {
  ...
  // lit `now` (ou une bande animée / sceneHumanH) · 'front' = statique dessiné PAR-DESSUS.
  // Avec pass='all' (défaut) les trois booléens valent true → ordre et appels STRICTEMENT
  // identiques à avant. Critère : l'appel (ou sa position) lit-il `now` ? → animé.
  const dBack = pass === 'all' || pass === 'back';
  const dAnim = pass === 'all' || pass === 'anim';
  ...
  if (id === "storytellers") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "storytellers", pass); return; }
    ...
    // Tout ce qui suit lit `now` (flammes/halo) ou s'en nourrit (alphas via `halo`,
    // lueurs de pages via `pglo`) : le livre et les personnages sont pris en sandwich
    // entre le halo du feu et leurs propres lueurs → classés ANIMÉS en bloc (dessinés
    // en direct, ordre inchangé — jamais faux, juste pas cachés).
    if (dAnim) {
  ...
  if (id === "scribes") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "scribes", pass); return; }
    ...
    if (propReady('scribes-prop-hall')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'scribes-prop-hall', 0.5, 0.53, 0.88, 0.73);
      return;
    }
    ...
    if (dFront) { // tables + porte : statiques dessinées APRÈS les bougies animées
    const nT = 2 + tier;
  ...
  if (id === "schools") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "schools", pass); return; }
    ...
    if (propReady('schools-prop-yard')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'schools-prop-yard', 0.5, 0.53, 0.88, 0.73);
      return;
    }
    ...
    if (dFront) { // colonnes + porte : statiques dessinées APRÈS le maître animé
  ...
  if (id === "academies") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "academies", pass); return; }
    ...
    if (propReady('academies-prop-yard')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'academies-prop-yard', 0.5, 0.53, 0.88, 0.73);
      return;
    }
    ...
    if (dFront) { // estrade + porte + liseré : statiques dessinés APRÈS le cercle animé
  ...
  if (id === "ancestral_cult") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "ancestral_cult", pass); return; }
    ...
    if (propReady('ancestralcult-prop')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'ancestralcult-prop', 0.5, 0.53, 0.88, 0.73);
      return;
    }
    ...
    // Halo de nuit + fidèles : statiques (pas de `now`) dessinés APRÈS la flamme animée
    if (dFront) {
    if (night > 0.2) {
  ...
  if (id === "observatories") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "observatories", pass); return; }
    ...
    if (propReady('observatories-prop-dial')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'observatories-prop-dial', 0.5, 0.53, 0.88, 0.73);
      return;
    }
    ...
    if (dAnim) { // fente + télescope + pulse : tout lit `now`
    ...
    if (dFront) { // rose des vents + annexe : statiques dessinés APRÈS le télescope animé
    if (tier >= 1) {
  ...
  if (id === "libraries") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "libraries", pass); return; }
    ...
    if (propReady('libraries-prop-archive')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'libraries-prop-archive', 0.5, 0.50, 0.88, 0.80);
      return;
    }
    ...
    for (let i = 0; i < 1 + tier; i++) {
      if (dBack) px(0.22 + i * 0.22, 0.66, 0.14, 0.06, "#d8c898");
    ...
    if (dFront) px(0.44, 0.60, 0.12, 0.20, "#2e1e0a"); // porte : statique dessinée APRÈS les lecteurs animés
    return;
  }
  if (id === "universities") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "universities", pass); return; }
    ...
    if (propReady('universities-prop-hall')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'universities-prop-hall', 0.5, 0.50, 0.88, 0.80);
      return;
    }
    const t2 = now || 0;
    ...
    if (band <= 3) {
      ...
      // (tout ce chemin est STATIQUE : aucun `now`)
      if (dBack) {
      // Pelouse de la cour
      ...
      // Tier 2 : coupole d'observatoire (toit) — fill+liseré indissociables du glow qui
      // respire → tout le bloc classé ANIMÉ (doute = animé, jamais faux).
      if (tier >= 2 && dAnim) {
      ...
      // Lueur nocturne (statique — pas de `now`) dessinée APRÈS les animations
      if (dFront && night > 0.25) {
      ...
      if (dAnim) { // LED + antennes (pylône fondu à sa diode qui pulse) + hologramme : tout lit `now`
  ...
  if (id === "printing_houses") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "printing_houses", pass); return; }
    ...
    if (propReady('printing-prop-workshop')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'printing-prop-workshop', 0.5, 0.50, 0.88, 0.80);
      return;
    }
    ...
    if (dAnim) { // barre de presse qui tourne
    ...
    if (dFront) { // feuilles/encriers/lampe : statiques dessinés APRÈS la presse animée
    ...
    if (tier >= 2) {
      // Cheminée : statique, disjointe de la presse → couche arrière (la fumée passe dessus)
      if (dBack) px(0.80, 0.10, 0.08, 0.20, "#3a2a18");
      if (dAnim) {
  ...
  if (id === "think_tanks") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "think_tanks", pass); return; }
    ...
    if (propReady('think-prop-council')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'think-prop-council', 0.5, 0.50, 0.88, 0.80);
      return;
    }
    ...
    if (dAnim) { // écrans qui pulsent, antennes à diode, hologrammes : tout lit `now`
  ...
  if (id === "aqueducts") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "aqueducts", pass); return; }
    ...
    if (aqStage >= 1 && propReady(eraPfx + '-outlet') && propReady(eraPfx + '-seg') && propReady(eraPfx + '-intake')) {
      ...
      for (let i = 0; i < spanX; i++) {
        ...
        const mcx = (i + 0.5) * cw;
        if (dBack) blitProp(ctx, ox, oy, sw, sh, eraPfx + '-' + mod, mcx, 0.59, cw, 1.5);
        if (dAnim && animReady('aqueduct-water-' + mod)) blitAnim(ctx, ox, oy, sw, sh, 'aqueduct-water-' + mod, now, mcx, waterCy, cw, 0.12);
      }
      if (flip) ctx.restore();
      return;
    }
    ...
    if (propReady('aqueduct-outlet') && propReady('aqueduct-seg') && propReady('aqueduct-intake')) {
      ...
      for (let i = 0; i < spanX; i++) {
        const mod = i === 0 ? 'outlet' : (i === spanX - 1 ? 'intake' : 'seg');
        const mcx = (i + 0.5) * cw;
        if (dBack) blitProp(ctx, ox, oy, sw, sh, 'aqueduct-' + mod, mcx, 0.59, cw, 1.5);
        if (dAnim && animReady('aqueduct-water-' + mod)) blitAnim(ctx, ox, oy, sw, sh, 'aqueduct-water-' + mod, now, mcx, 0.59, cw, 1.5);
      }
      if (flip) ctx.restore();
      return;
    }
  ...
  if (id === "watch") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "watch", pass); return; }
    ...
    if (animReady('watch-fire') && propReady('watch-back')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'watch-back', 0.5, 0.40, 0.78, 0.94);
      if (dAnim) blitAnim(ctx, ox, oy, sw, sh, 'watch-fire', now, 0.5, 0.40, 0.78, 0.94);
      return;
    }
    if (propReady('watch-prop')) {
      if (dBack) blitProp(ctx, ox, oy, sw, sh, 'watch-prop', 0.5, 0.40, 0.78, 0.94);
      return;
    }
    ...
    if (ei2 >= 11) {
      ...
      if (dAnim) { // caméra qui balaie + écrans + faisceau : tout lit `now`
      ...
      // Brasier (statique) posé PAR-DESSUS sa lueur animée
      if (dFront) { ctx.fillStyle = "#c09030"; ctx.beginPath(); ctx.arc(ox+sw*0.5, oy+sh*0.19, sw*0.065, 0, Math.PI*2); ctx.fill(); }
    } else {
      ...
      // Halo de nuit (statique — pas de `now`) dessiné APRÈS les flammes animées
      if (dFront && night > 0.2) { const hg=ctx.createRadialGradient(ox+sw*0.5,oy+sh*0.22,0,ox+sw*0.5,oy+sh*0.22,sw*0.3); hg.addColorStop(0,`rgba(255,150,20,${(0.18*night).toFixed(2)})`); hg.addColorStop(1,"rgba(255,80,5,0)"); ctx.fillStyle=hg; ctx.beginPath(); ctx.arc(ox+sw*0.5,oy+sh*0.22,sw*0.3,0,Math.PI*2); ctx.fill(); }
    }
    return;
  }
  if (id === "sewers") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "sewers", pass); return; }
    ...
    if (ei2 >= 11) {
      ...
      if (dFront) { // poste de commande : statique dessiné APRÈS les diodes/flux animés
      px(0.08, 0.32, 0.28, 0.10, "#1e2434");
      for (let i=0; i<3; i++) ctx.fillRect(ox+sw*(0.11+i*0.08), oy+sh*0.35, sw*0.05, sh*0.04);
      }
    } else if (band2 >= 3) {
      if (dBack) { px(0.06, 0.46, 0.88, 0.30, "#3a3020"); px(0.10, 0.56, 0.80, 0.16, "#2a2015"); }
      const nG=2+tier;
      for (let i=0; i<nG; i++) {
      ...
      // Canal + flux : le canal (statique) est pris en sandwich SOUS la vague → animé en bloc
      if (dAnim) {
  ...
  if (id === "bureaucracy") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "bureaucracy", pass); return; }
    ...
    const buStage = engineStage(ei);
    {
      ...
      if (propReady(bub)) { drawStagePix(ctx, ox, oy, sw, sh, bub, now, buStage === 3 ? '218,226,255' : '255,186,90', { ph: 2.0 }, pass); return; }
    }
    ...
    if (ei2 >= 11) {
      ...
      if (dFront) px(0.36, 0.06, 0.28, 0.08, "#d4a017"); // bandeau doré : statique APRÈS les écrans animés
    } else if (ei2 >= 5) {
      if (dBack) {
      ...
      if (dFront) { // bandeau/porte/fenêtres : statiques dessinés APRÈS le tampon animé
  ...
  if (id === "courthouses") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "courthouses", pass); return; }
    ...
    const coStage = engineStage(ei);
    {
      ...
      if (propReady(cob)) { drawStagePix(ctx, ox, oy, sw, sh, cob, now, coStage === 3 ? '255,208,140' : '255,186,90', { ph: 1.3 }, pass); return; }
    }
    ...
    if (ei2 >= 11) {
      if (dBack) { // chemin ENTIÈREMENT statique (aucun `now`)
  ...
  if (id === "public_works") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "public_works", pass); return; }
    ...
    const pwStage = engineStage(ei);
    {
      ...
      if (propReady(pwb)) { drawStagePix(ctx, ox, oy, sw, sh, pwb, now, pwStage === 3 ? '110,205,235' : '255,176,78', { ph: 0.7 }, pass); return; }
    }
    ...
    if (ei2 >= 11) {
      ...
      if (dFront) { // engin + matériaux : statiques dessinés APRÈS la charge animée
      ...
      if (dFront) { // bloc posé + ouvriers : statiques dessinés APRÈS la charge animée
      ...
      if (dAnim) for (let i=0; i<2+tier; i++) {
      ...
      if (dFront) { // corde du palan : statique dessinée APRÈS les ouvriers animés
  ...
  if (id === "ministries") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "ministries", pass); return; }
    // ── ÉVOLUTION 4 STADES (2026-07-05, INFRA) : stade 0 procédural conservé (ci-dessous),
    ...
    const miStage = engineStage(ei);
    {
      ...
      if (propReady(mib)) { drawStagePix(ctx, ox, oy, sw, sh, mib, now, miStage === 3 ? '225,228,255' : '255,186,90', { ph: 0.4 }, pass); return; }
    }
    // Ministères : salle du conseil → palais → ministère moderne
    // (les 3 chemins procéduraux sont ENTIÈREMENT statiques : aucun `now`)
    const ei2 = (CM.layout && CM.layout.counts) ? CM.layout.counts.eraIndex : 5;
    if (!dBack) return;
    if (ei2 >= 11) {
  ...
  if (id === "archive_grids") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "archive_grids", pass); return; }
    ...
    const arStage = engineStage(ei);
    {
      ...
      if (propReady(arb)) { drawStagePix(ctx, ox, oy, sw, sh, arb, now, arStage === 3 ? '90,225,205' : '255,182,84', { ph: 2.6 }, pass); return; }
    }
    ...
    if (ei2 >= 10) {
      ...
      if (dAnim) { // flux + antenne à balise : tout lit `now` (l'antenne statique reste liée à sa diode)
      ...
      if (!dBack) return; // chemin ENTIÈREMENT statique (aucun `now`)
  ...
  if (id === "ruin_architects") {
    if (band >= 7) { cosmicSavoir(ctx, ox, oy, sw, sh, px, band, now, "ruin_architects", pass); return; }
    ...
    const ruStage = engineStage(ei);
    {
      ...
      if (propReady(rub)) { drawStagePix(ctx, ox, oy, sw, sh, rub, now, ruStage === 3 ? '120,215,205' : '255,180,90', { ph: 3.1 }, pass); return; }
    }
    ...
    if (dAnim) { // blueprint qui pulse (fond + traits liés à `bpA`)
    ...
    if (dFront) { // architectes + échafaudages : statiques dessinés APRÈS le blueprint animé
...
function drawEngineSprite(t, x, y, w, h, now, pass = 'all') {
  ...
  drawEngineSpriteCore(t, x, y, w, h, now, pass);
}
