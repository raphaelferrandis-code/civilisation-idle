...
export const buildings = [
  {
    ...
    // Rythme early game (rééquilibrage 2026-07) : food 2.4 → 0.8 pour que le
    // 2e cueilleur demande ~10 s (avant : un achat toutes les ~1,4 s dès t=0).
    // Le rythme de toute la chaîne est porté par les COÛTS : aucun bâtiment n'a
    // de condition de déblocage affichée — ils se révèlent quand l'économie
    // approche leur coût (cf. isUnlocked, apparition économique) ; seuls les
    // cycles (unlockCycles) verrouillent explicitement. Les bases d'ère 1 sont
    // CALIBRÉES PAR SIMULATION (joueur glouton) pour des écarts de déblocage
    // CROISSANTS : ~1m30, 2m30, 3m, 7m, … jusqu'à ~2h+ — champs et académies
    // débordent volontairement sur les cycles suivants (ruines requises).
    base: 10,
    scale: 1.19,
    currency: "food",
    pop: 0.18,
