...
## 1. Bâtiments ACHETABLES (sprite moteur unique)

> La plupart sont identiques à tous les âges (`tier` = richesse). Exceptions
> notées « 4 stades » : silhouette qui change par ère (`stage = ei<10/<20/<30/sinon`),
> comme `foragers`. `river_ports` et `water_mills` sont en plus UNIQUES et posés
> sur la berge (eau au sud, cf. branche de pose dédiée dans `layout.js`).

### City — `src/game/map/cityEngineSprites.js`
| Bâtiment | id | Ligne | Notes |
|---|---|---|---|
| Cueilleurs | `foragers` | 5 | 4 stades |
| Entrepôts | `granaries_city` | 99 | |
| Caravanes | `caravans` | 177 | |
| Marchés | `markets` | 241 | |
| Guildes | `guilds` | 314 | |
| Champs | `irrigated_fields` | 366 | unique (ceinture) |
| Ports | `river_ports` | 1500 | unique, berge, 4 stades |
| Moulins | `water_mills` | 1747 | unique, berge, 4 stades |
| Hôtels des monnaies | `mint_houses` | 1857 | |
