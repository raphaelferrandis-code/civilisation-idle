...
| **S12** districts | ✅ **livré ce jour**, ⛔ **son art RETIRÉ le soir même** | repères civiques (masses = scènes moteur apparentées) gardés. ⚠ v2 : **UN monument par genre** près du cœur — la v1 en posait 18, Raph : « ça alourdit ». ⛔⛔ L'**art dédié des 10 genres** a été généré puis RETIRÉ : « les bâtiments créés sont de face ». Demandés en `low top-down`, le modèle a rendu des façades frontales — le défaut exact que la campagne de juillet (`building-frontview-regen`) avait déjà corrigé. **Sa leçon, non appliquée : faire l'audit visuel SOI-MÊME sur planche-contact, pas juger 3 sprites sur 10.** Toute reprise de l'art passe par une planche |
